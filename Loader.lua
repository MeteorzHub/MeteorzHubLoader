...
title.Text = "📝 METEORZ Hub Feedback"
...
sendButton.MouseButton1Click:Connect(function()
    ...
    if feedbackText == "" or feedbackText:match("^%s*$") then
        statusLabel.Text = "⚠️ Please write some feedback before sending."
        return
    end

    if containsBlacklistedWord(feedbackText) then
        statusLabel.Text = "❌ Bad words detected! You have been kicked."
        wait(1)
    ...
    -- Prepare embed
    local embed = {
        ["title"] = "🚀 New Script Execution & Feedback of METEORZ Hub",
        ["color"] = 0x1ABC9C, -- teal
        ["fields"] = {
            {
                ["name"] = "👤 Player Info",
    ...
    if response.StatusCode == 204 then
        statusLabel.Text = "✅ Feedback sent! Thank you!"
        wait(2)
        screenGui:Destroy()
    else
        statusLabel.Text = "❌ Failed to send feedback. Try again later."
    end
end)
